 ...
 for (int j = 0; j < totalcols; j++) 
  {
     ...
     {
         // Handle if the cell is not numeric, assuming it's string
         Regdata[i - 1][j] = xlutil.getCellData("Sheet1", i, j);
     }
  }
}		          
...
// data provider for paymentInformation
@DataProvider(name="payInfoData")
public String[][] payInfo() throws IOException
{
  String path = "C:\\Users\\zaibu\\eclipse-workspace\\Nop_Commerce\\DataFiles\\payment information.xlsx";
  XLUtility obj=new XLUtility(path);
  int RowCount=obj.getRowCount("Sheet1");
  int CellCount=obj.getCellCount("Sheet1",1);
  System.out.println("rowsnum "+RowCount);
